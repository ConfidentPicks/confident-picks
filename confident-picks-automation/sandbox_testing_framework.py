...
def get_comprehensive_feature_columns():
    """Get all comprehensive feature columns"""
    return [
...
def train_comprehensive_model(df):
    """Train comprehensive model with multiple algorithms"""
    print("Training comprehensive model...")
    
    # Create features
    df_with_features = create_comprehensive_features(df)
    
    # Get feature columns
    feature_columns = get_comprehensive_feature_columns()
...
def create_test_data():
    ...
    # Basic features
    away_win_pct = np.random.beta(2, 2, n_games)
    ...
    home_win_prob = (
        home_win_pct * 0.3 +
        (home_points_for - away_points_for) * 0.2 +
        (away_points_against - home_points_against) * 0.2 +
        (home_win_pct - away_win_pct) * 0.1 +
        np.random.normal(0, 0.1, n_games) * 0.2
    )
    ...
    df = pd.DataFrame({
        ...
        'away_avg_points_for': away_points_for,
        'home_avg_points_for': home_points_for,
        'away_avg_points_against': away_points_against,
        'home_avg_points_against': home_points_against,
        'away_rest': np.random.randint(3, 14, n_games),
        'home_rest': np.random.randint(3, 14, n_games),
        'temp': np.random.normal(70, 20, n_games),
        'wind': np.random.exponential(5, n_games),
        'away_score': np.where(y == 0, np.random.randint(20, 35, n_games), np.random.randint(10, 25, n_games)),
        'home_score': np.where(y == 1, np.random.randint(20, 35, n_games), np.random.randint(10, 25, n_games))
    })
    
    return df

def main():
    """Main function to test the comprehensive model"""
    ...
    # Train model
    model, features, score, results = train_comprehensive_model(test_df)
