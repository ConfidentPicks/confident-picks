...
"""
Add Comprehensive Prediction Analysis Columns
===========================================
...
def main():
    ...
    for i, header in enumerate(headers):
        if 'home_team' in header.lower():
            ...
            col_indices['predicted_team'] = i
        elif 'spread_cover_away' in header.lower():
            col_indices['spread_cover_away'] = i
        elif 'predicted_cover_away' in header.lower():
            col_indices['predicted_cover_away'] = i
        elif 'actual_total' in header.lower():
            col_indices['actual_total'] = i
    ...
    if 'predicted_team' in col_indices:
        col_letter = get_column_letter(col_indices['predicted_team'])
    ...
    if 'spread_cover_away' in col_indices:
        col_letter = get_column_letter(col_indices['spread_cover_away'])
    ...
    # 4. Predicted_Cover_Away formula
    if 'predicted_cover_away' in col_indices:
        col_letter = get_column_letter(col_indices['predicted_cover_away'])
        formula = f'=IF({get_column_letter(col_indices["predicted_away_score"])}2+{get_column_letter(col_indices["spread_line"])}2>{get_column_letter(col_indices["predicted_home_score"])}2,"YES","NO")'
        formula_updates.append({
            'range': f"upcoming_games!{col_letter}2",
            'values': [[formula]]
        })
    ...
    if 'actual_total' in col_indices:
        col_letter = get_column_letter(col_indices['actual_total'])
    ...
    if 'predicted_home_score' in col_indices:
        col_letter = get_column_letter(col_indices['predicted_home_score'])
    ...
    if 'predicted_away_score' in col_indices:
        col_letter = get_column_letter(col_indices['predicted_away_score'])
        formula = f'=({get_column_letter(col_indices["predicted_total"])}2-{get_column_letter(col_indices["predicted_spread"])}2)/2'
        formula_updates.append({
            'range': f"upcoming_games!{col_letter}2",
            'values': [[formula]]
    ...
    print(f"   - Predicted_Cover_Away: =IF(predicted_away_score+spread_line>predicted_home_score,\"YES\",\"NO\")")
