...
def create_historical_data():
    """Create historical game data (limited to fit Google Sheets)"""
    print("📊 Creating historical game data...")
    ...
    # Generate games for each season (limited to fit in Google Sheets)
    for season in [2021, 2022, 2023, 2024]:
        print(f"Creating {season} season data...")
        
        # Generate ~200 games per season to fit in Google Sheets (1000 row limit)
        for week in range(1, 13):  # 12 weeks instead of 17
    ...
    print(f"✅ Created {len(df)} historical games")
...
def main():
    print("🏈 CREATING AND UPLOADING HISTORICAL GAME DATA")
    print("=" * 60)
    ...
    if df.empty:
        print("❌ Failed to create historical data")
        return False
    ...
    if upload_success:
        print("\n✅ HISTORICAL GAMES DATA SUCCESSFULLY CREATED AND UPLOADED!")
