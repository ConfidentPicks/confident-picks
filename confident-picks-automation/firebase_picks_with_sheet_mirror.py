...
"""
FIREBASE PICK CARDS WITH SHEET MIRROR
=====================================

This script creates pick cards, pushes to Firebase, AND mirrors the data
to a Google Sheet for easy viewing and editing.

WORKFLOW:
1. Generate pick cards from Google Sheets data
2. Push to Firebase (test_picks collection)
3. Mirror data to Google Sheet (Firebase Picks tab)
4. Allow easy editing in the sheet
5. Sync changes back to Firebase

USAGE:
- Run manually: python firebase_picks_with_sheet_mirror.py
- Integrate: Import and call create_picks_with_sheet_mirror()
"""
...
def create_firebase_picks_sheet():
    """Create or update the Firebase Picks sheet"""
    print("Creating/updating Firebase Picks sheet...")
    ...
    try:
        ...
        firebase_picks_sheet = None
        for sheet in sheets:
            if sheet['properties']['title'] == 'Firebase Picks':
                firebase_picks_sheet = sheet
                break
        
        if not firebase_picks_sheet:
            # Create new sheet
            requests = [{
                'addSheet': {
                    'properties': {
                        'title': 'Firebase Picks',
                        'gridProperties': {
                            'rowCount': 1000,
                            'columnCount': 16
                        }
            ...
            print("Created Firebase Picks sheet")
        else:
            print("Firebase Picks sheet already exists")
            
    except Exception as e:
        print(f"Error creating sheet: {e}")

def push_picks_to_sheet(pick_cards):
    """Push pick cards to Google Sheet"""
    print(f"Pushing {len(pick_cards)} pick cards to Google Sheet...")
    ...
    for pick_card in pick_cards:
        row = [
            ...
            pick_card['pick_type'],
            pick_card['pick_team'],
            pick_card['confidence_level'],
            pick_card['model_confidence'],
            pick_card['team_accuracy'],
            pick_card['reasoning_part1'],
            pick_card['reasoning_part2'],
            pick_card['reasoning_part3'],
            pick_card['game_date'],
            pick_card['status'],
            pick_card['last_updated'],
            'Synced'
        ]
        sheet_data.append(row)
    
    # Clear existing data and add new data
    range_name = 'Firebase Picks!A1:P1000'
    ...
    service.spreadsheets().values().update(
        spreadsheetId=SPREADSHEET_ID,
        range='Firebase Picks!A1',
        valueInputOption='RAW',
        body={'values': sheet_data}
    ).execute()
    
    print("Successfully pushed pick cards to Google Sheet")

def create_picks_with_sheet_mirror():
    """Create pick cards, push to Firebase, and mirror to Google Sheet"""
    print("=" * 80)
    print("CREATING PICKS WITH SHEET MIRROR")
    print("=" * 80)
    
    service = get_sheets_service()
    db = get_firestore_client()
    
    # Create Firebase Picks sheet
    create_firebase_picks_sheet()
    ...
    for i, (game_row, pred_row) in enumerate(zip(games_data, predictions_data)):
        ...
        if len(game_row) >= 2 and len(pred_row) >= 3:
            ...
            if predicted_winner in high_performers:
                ...
                doc_id = f"{game.replace(' @ ', '_vs_').replace(' ', '_')}_moneyline_{predicted_winner}"
            ...
            for team in [away_team, home_team]:
                if team in high_performers and team != predicted_winner:
                    ...
                    if spread_accuracy >= 50:
                        ...
                        doc_id = f"{game.replace(' @ ', '_vs_').replace(' ', '_')}_spread_{team}"
                        doc_id = doc_id.replace(' ', '_').replace('@', 'vs')
    ...
    # Push to Firebase
    print(f"Pushing {len(pick_cards)} pick cards to Firebase...")
    
    for pick_card in pick_cards:
        try:
            doc_ref = db.collection('test_picks').document(pick_card['doc_id'])
            doc_ref.set(pick_card)
        except Exception as e:
            print(f"Error adding {pick_card['game']}: {str(e)}")
    
    # Push to Google Sheet
    push_picks_to_sheet(pick_cards)
    
    print(f"\n{'='*80}")
    print("SUMMARY")
    print(f"{'='*80}")
    print(f"Total pick cards created: {len(pick_cards)}")
    print(f"High confidence picks: {len([p for p in pick_cards if p['confidence_level'] == 'HIGH'])}")
    print(f"Medium confidence picks: {len([p for p in pick_cards if p['confidence_level'] == 'MEDIUM'])}")
    print(f"All pick cards pushed to Firebase 'test_picks' collection")
    print(f"All pick cards mirrored to Google Sheet 'Firebase Picks' tab")
    print(f"\nYou can now edit picks directly in the Google Sheet!")

if __name__ == "__main__":
    create_picks_with_sheet_mirror()
