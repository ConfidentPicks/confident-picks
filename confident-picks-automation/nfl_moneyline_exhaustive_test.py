#!/usr/bin/env python3
"""NFL Moneyline (Winner) Exhaustive Testing - Target: 32 teams at 70%+ accuracy"""
import pandas as pd
import numpy as np
...
PROGRESS_FILE = 'nfl_moneyline_progress.json'
...
def create_features(df):
    ...
    for col in ['away_score', 'home_score', 'away_moneyline', 'home_moneyline']:
    ...
    for team in set(df['away_team'].unique().tolist() + df['home_team'].unique().tolist()):
        if pd.notna(team) and team != '':
            team_stats[team] = {
                ...
                'form': [], 'streak': 0, 'home_wins': 0, 'home_games': 0,
    ...
    for idx, row in df.iterrows():
        ...
        df.at[idx, 'away_road_pct'] = away_stats.get('away_wins', 0) / max(away_stats.get('away_games', 0), 1)
        df.at[idx, 'home_home_pct'] = home_stats.get('home_wins', 0) / max(home_stats.get('home_games', 0), 1)
        ...
        if pd.notna(row['away_score']) and pd.notna(row['home_score']):
            try:
                ...
                team_stats[away_team]['away_wins'] += away_won
                team_stats[away_team]['form'].append(away_won)
                if away_won:
                ...
                team_stats[home_team]['home_wins'] += home_won
                team_stats[home_team]['form'].append(home_won)
                if home_won:
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("NFL MONEYLINE (WINNER) EXHAUSTIVE TESTING")
    ...
    feature_cols = ['away_win_pct', 'home_win_pct', 'away_point_diff', 'home_point_diff',
                    ...
                    'away_road_pct', 'home_home_pct', 'win_pct_diff', 'point_diff_diff',
                    'form_diff', 'streak_diff', 'home_field']
    
    df_clean = df.dropna(subset=feature_cols + ['home_wins', 'away_team', 'home_team'])
    print(f"Clean data: {len(df_clean)} games\n")
    
    X = df_clean[feature_cols]
    y = df_clean['home_wins']
                    ...
                    for model_params in model_config['params']:
                        try:
                            ...
                            if count_70plus > best_count:
                                ...
                                model_name = f"NFL-ML-v{model_params.get('n_estimators', 100)}-{model_config['name'][:2]}"
