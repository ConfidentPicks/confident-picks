...
def create_pick_cards_60_percent():
    """Create pick cards for all 10/26/2025 games with 60% threshold"""
    print("=" * 80)
    print("PICK CARDS FOR 10/26/2025 - 60% THRESHOLD")
    print("=" * 80)
    ...
    for i, (game_row, pred_row) in enumerate(zip(games_data, predictions_data)):
        ...
        if len(game_row) >= 2 and len(pred_row) >= 3:
            ...
            if predicted_winner in high_performers:
                ...
                if stats['moneyline'] >= 70:
                    confidence_level = "HIGH"
                elif stats['moneyline'] >= 60:
                ...
                reasoning = f"""
{confidence_level} CONFIDENCE PICK:
• Model predicts {predicted_winner} to win with {winner_confidence} confidence
• {predicted_winner} has {stats['moneyline']}% moneyline accuracy this season (60%+ threshold)
...
LOW CONFIDENCE PICK:
• Model predicts {predicted_winner} to win with {winner_confidence} confidence
• Neither team meets 60%+ accuracy threshold
• Consider avoiding or use with caution"""
    ...
    # Summary
    print(f"\n{'='*80}")
    print("SUMMARY - 60% THRESHOLD")
    print(f"{'='*80}")
    print(f"HIGH CONFIDENCE PICKS (70%+): {len(high_confidence_picks)}")
    for pick in high_confidence_picks:
        print(f"  • {pick['game']}: {pick['pick']}")
    
    print(f"\nMEDIUM CONFIDENCE PICKS (60-69%): {len(medium_confidence_picks)}")
    for pick in medium_confidence_picks:
        print(f"  • {pick['game']}: {pick['pick']}")
    
    print(f"\nLOW CONFIDENCE PICKS (<60%): {len(low_confidence_picks)}")
    for pick in low_confidence_picks:
        print(f"  • {pick['game']}: {pick['pick']}")
    
    total_publishable = len(high_confidence_picks) + len(medium_confidence_picks)
    print(f"\nTOTAL PUBLISHABLE PICKS: {total_publishable} out of 12 games ({total_publishable/12*100:.1f}%)")

if __name__ == "__main__":
    create_pick_cards_60_percent()
