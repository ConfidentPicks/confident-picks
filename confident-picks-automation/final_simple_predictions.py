...
"""
Final Simple Predictions
=======================

This script adds predictions to the upcoming_games tab without DataFrame issues
"""
...
def main():
    ...
    print("🏈 Final Simple NFL Predictions")
    print("=" * 40)
    ...
    # Simple predictions based on team names
    ...
    for idx, row in enumerate(data_rows):
        ...
        if home_team and away_team:
            ...
            strong_teams = ['KC', 'BUF', 'MIA', 'BAL', 'SF', 'DAL', 'PHI', 'GB']
            ...
            if 'predicted_winner' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['predicted_winner'])}{row_num}", 'values': [[predicted_winner]]})
            if 'confidence_score' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['confidence_score'])}{row_num}", 'values': [[confidence]]})
            if 'home_win_probability' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['home_win_probability'])}{row_num}", 'values': [[home_win_prob]]})
            if 'away_win_probability' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['away_win_probability'])}{row_num}", 'values': [[away_win_prob]]})
            if 'betting_recommendation' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['betting_recommendation'])}{row_num}", 'values': [[recommendation]]})
            if 'betting_value' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['betting_value'])}{row_num}", 'values': [[confidence - 0.5]]})
            if 'model_last_updated' in col_indices:
                updates.append({'range': f"upcoming_games!{chr(65+col_indices['model_last_updated'])}{row_num}", 'values': [[datetime.now().strftime('%Y-%m-%d %H:%M:%S')]]})
