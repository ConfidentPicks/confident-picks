...
"""
Update Live Picks Sheet with Predictions
=======================================

This script updates the live_picks_sheets tab with predictions
"""
...
class LivePicksUpdater:
    ...
    def update_live_picks_sheet(self, live_picks_df, predictions):
        """Update the live_picks_sheets with predictions"""
        try:
            print("\n📝 Writing predictions to live_picks_sheets...")
            
            # Get the current sheet structure
            result = self.service.spreadsheets().values().get(
                spreadsheetId=self.spreadsheet_id,
            ...
            # Find the column indices for prediction columns
            col_mapping = {}
            for i, header in enumerate(headers):
                if 'predicted_winne' in header.lower():
                    col_mapping['predicted_winner'] = i
                elif 'confidence_scor' in header.lower():
                    col_mapping['confidence_score'] = i
                elif 'home_win_probi' in header.lower():
                    col_mapping['home_win_prob'] = i
                elif 'away_win_proba' in header.lower():
                    col_mapping['away_win_prob'] = i
                elif 'betting_recomm' in header.lower():
                    col_mapping['betting_recommendation'] = i
                elif 'betting_value' in header.lower():
                    col_mapping['betting_value'] = i
            
            print(f"Found prediction columns: {col_mapping}")
            
            # Update each row with predictions
            for idx, row in live_picks_df.iterrows():
                ...
                if game_id in predictions:
                    ...
                    for col_name, col_idx in col_mapping.items():
                        if col_name == 'predicted_winner':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['predicted_winner']]]})
                        elif col_name == 'confidence_score':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['confidence_score']]]})
                        elif col_name == 'home_win_prob':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['home_win_probability']]]})
                        elif col_name == 'away_win_prob':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['away_win_probability']]]})
                        elif col_name == 'betting_recommendation':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['betting_recommendation']]]})
                        elif col_name == 'betting_value':
                            updates.append({'range': f"live_picks_sheets!{chr(65+col_idx)}{idx+2}", 'values': [[pred['betting_value']]]})
                    ...
                    if updates:
                        ...
                        print(f"  ✓ Updated {matchup}: {pred['predicted_winner']} ({pred['confidence_score']:.1%} confidence)")
            
            print("✅ Successfully updated live_picks_sheets with predictions!")
            
        except Exception as e:
            print(f"❌ Error updating live_picks_sheets: {e}")
    ...
    def run_live_picks_update(self):
        """Run the live picks update system"""
        print("🏈 Live Picks Sheet Prediction Update")
        print("=" * 60)
        ...
        # Load live picks sheet
        print("\n🔮 Making predictions for live picks...")
        live_picks_df = self.load_sheet_data("live_picks_sheets")
        
        if live_picks_df.empty:
            print("⚠️ No live picks found")
            return
        
        # Make predictions for all games
        predictions = {}
        for idx, game in live_picks_df.iterrows():
            prediction = self.predict_game(game)
            if prediction:
                game_id = game.get('game_id', idx)
                predictions[game_id] = prediction
        
        # Update the live picks sheet
        self.update_live_picks_sheet(live_picks_df, predictions)
        
        print("\n" + "=" * 60)
        print("✅ Live picks update complete!")
        print(f"\n📊 Updated {len(predictions)} games in live_picks_sheets")
        print("   Check your live_picks_sheets tab for predictions!")
...
def main():
    ...
    # Initialize and run
    system = LivePicksUpdater(credentials_path, spreadsheet_id)
    system.run_live_picks_update()
