...
def load_historical_data():
    """Load 2021-2024 historical data for training"""
    print("📊 Loading 2021-2024 historical data...")
    
    service = get_sheets_service()
    
    # Try to load from historical spreadsheet
    try:
        range_name = f"'historical_game_results_2021_2024'!A:ZZ"
        result = service.spreadsheets().values().get(
            spreadsheetId=HISTORICAL_SPREADSHEET_ID, range=range_name).execute()
        ...
        if values:
            ...
            print(f"✅ Loaded {len(df)} historical games from 2021-2024")
            return df
        else:
...
def load_2025_data():
    """Load 2025 current season data for testing"""
    print("📊 Loading 2025 current season data...")
    
    service = get_sheets_service()
    range_name = f"'upcoming_games'!A:ZZ"
    result = service.spreadsheets().values().get(
...
def create_team_features(df):
    """Create team-based features without using ESPN FPI"""
...
def main():
    print("🏈 HISTORICAL TRAINING WINNER PREDICTION MODEL")
    print("=" * 70)
