...
def load_historical_data():
    """Load 2021-2024 historical data from current spreadsheet"""
    print("📊 Loading 2021-2024 historical data...")
    
    # Try to load from historical_game_results_2021_2024 sheet
    try:
        df = load_sheet_data(SPREADSHEET_ID, 'historical_game_results_2021_2024')
        if not df.empty:
            print(f"✅ Loaded {len(df)} historical games from historical_game_results_2021_2024")
            return df
    except Exception as e:
        print(f"❌ Could not load historical_game_results_2021_2024: {e}")
    
    # If that fails, try to load from other sheets
    try:
        df = load_sheet_data(SPREADSHEET_ID, 'team_stats_2021_2024')
        if not df.empty:
            print(f"✅ Loaded {len(df)} historical games from team_stats_2021_2024")
            return df
    except Exception as e:
        print(f"❌ Could not load team_stats_2021_2024: {e}")
...
def load_2025_data():
    ...
    print(f"✅ Loaded {len(df)} games from 2025 season")
    return df

def create_team_features(df):
    """Create team-based features from historical data"""
    print("🔧 Creating team features from historical data...")
...
def main():
    print("🏈 CORRECT HISTORICAL TRAINING WINNER PREDICTION MODEL")
    print("=" * 70)
    ...
    historical_with_features = create_team_features(historical_completed)
    ...
    df_2025_with_features = create_team_features(completed_2025)
