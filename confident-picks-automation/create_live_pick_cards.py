...
def generate_pick_reasoning(pick_type, team, predicted_winner, winner_confidence, team_accuracy, spread_accuracy=None):
    """Generate standardized reasoning for pick cards"""
    
    if pick_type == "moneyline":
        confidence_level = "HIGH" if team_accuracy >= 70 else "MEDIUM"
        reasoning = f"""
{confidence_level} CONFIDENCE MONEYLINE PICK:
• Model Prediction: {team} to win ({winner_confidence} confidence)
• Team Performance: {team_accuracy}% moneyline accuracy this season
• Signal Alignment: Model and performance both favor {team}
• Confidence Level: {confidence_level} (everything aligned)"""
        ...
        reasoning = f"""
MEDIUM CONFIDENCE SPREAD PICK:
• Model Prediction: {predicted_winner} to win ({winner_confidence} confidence)
• Team Performance: {team} has {spread_accuracy}% spread {spread_type} accuracy
• Signal Conflict: Model favors {predicted_winner}, but {team} excels at covering spreads
• Confidence Level: MEDIUM (conflicting signals)"""
    
    return reasoning.strip()

def create_live_pick_cards():
    """Create pick cards and push to Firebase live_picks collection"""
    print("=" * 80)
    print("CREATING LIVE PICK CARDS FOR FIREBASE")
    print("=" * 80)
    ...
    for i, (game_row, pred_row) in enumerate(zip(games_data, predictions_data)):
        ...
        if len(game_row) >= 2 and len(pred_row) >= 3:
            ...
            if predicted_winner in high_performers:
                ...
                reasoning = generate_pick_reasoning(
                    "moneyline", 
                    predicted_winner, 
                    predicted_winner, 
                    winner_confidence, 
            ...
            for team in [away_team, home_team]:
                if team in high_performers and team != predicted_winner:
                    ...
                    if spread_accuracy >= 50:
                        ...
                        pick_cards.append(pick_card)
                        print(f"SPREAD PICK: {team} Spread {spread_type.title()} (MEDIUM confidence)")
            
            if not any(pick['game'] == game for pick in pick_cards):
    ...
    for i, pick_card in enumerate(pick_cards, 1):
        try:
            ...
            # Add to live_picks collection
            doc_ref = db.collection('live_picks').document(doc_id)
            doc_ref.set(pick_card)
            
            print(f"{i}. Added to Firebase: {pick_card['game']} - {pick_card['pick_team']} {pick_card['pick_type']}")
            print(f"   Confidence: {pick_card['confidence_level']}")
    ...
    print(f"All pick cards pushed to Firebase 'live_picks' collection")

if __name__ == "__main__":
    create_live_pick_cards()
