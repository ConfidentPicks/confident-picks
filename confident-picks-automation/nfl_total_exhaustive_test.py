#!/usr/bin/env python3
"""NFL Total (Over/Under) Exhaustive Testing - Target: 32 teams at 70%+ accuracy"""
import pandas as pd
import numpy as np
...
PROGRESS_FILE = 'nfl_total_progress.json'
...
def create_features(df):
    ...
    for col in ['away_score', 'home_score', 'total', 'total_line']:
    ...
    for team in set(df['away_team'].unique().tolist() + df['home_team'].unique().tolist()):
        if pd.notna(team) and team != '':
            team_stats[team] = {
                ...
                'form': [], 'streak': 0, 'over_count': 0, 'total_games': 0
            }
    ...
    for idx, row in df.iterrows():
        ...
        df.at[idx, 'away_ppg'] = away_stats.get('points_for', 0) / max(away_stats.get('games', 0), 1)
        df.at[idx, 'home_ppg'] = home_stats.get('points_for', 0) / max(home_stats.get('games', 0), 1)
        df.at[idx, 'away_papg'] = away_stats.get('points_against', 0) / max(away_stats.get('games', 0), 1)
        df.at[idx, 'home_papg'] = home_stats.get('points_against', 0) / max(home_stats.get('games', 0), 1)
        ...
        df.at[idx, 'away_over_pct'] = away_stats.get('over_count', 0) / max(away_stats.get('total_games', 0), 1)
        df.at[idx, 'home_over_pct'] = home_stats.get('over_count', 0) / max(home_stats.get('total_games', 0), 1)
        df.at[idx, 'combined_ppg'] = df.at[idx, 'away_ppg'] + df.at[idx, 'home_ppg']
        df.at[idx, 'combined_papg'] = df.at[idx, 'away_papg'] + df.at[idx, 'home_papg']
        
        # Update stats after game
        if pd.notna(row['away_score']) and pd.notna(row['home_score']) and pd.notna(row['total_line']):
            try:
                away_score, home_score, total_line = float(row['away_score']), float(row['home_score']), float(row['total_line'])
                game_total = away_score + home_score
                
                # Over/Under (compare to betting line, not final score)
                is_over = 1 if game_total > total_line else 0
                
                for team, score, opp_score in [(away_team, away_score, home_score), 
                    ...
                    team_stats[team]['over_count'] += is_over
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("NFL TOTAL (OVER/UNDER) EXHAUSTIVE TESTING")
    ...
    # Create target variable (game goes over the betting line)
    # Use 'total_line' (betting line) NOT 'total' (actual final score)
    df['is_over'] = ((df['away_score'] + df['home_score']) > df['total_line']).astype(int)
    
    # Feature columns
    feature_cols = ['away_win_pct', 'home_win_pct', 'away_ppg', 'home_ppg',
                    'away_papg', 'home_papg', 'away_form', 'home_form',
                    'away_streak', 'home_streak', 'away_over_pct', 'home_over_pct',
                    'combined_ppg', 'combined_papg', 'total']
    
    df_clean = df.dropna(subset=feature_cols + ['is_over', 'away_team', 'home_team'])
                    ...
                    for model_params in model_config['params']:
                        try:
                            ...
                            if count_70plus > best_count:
                                ...
                                model_name = f"NFL-TOT-v{model_params.get('n_estimators', 100)}-{model_config['name'][:2]}"
